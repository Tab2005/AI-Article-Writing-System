...
class KalpaService:
    ...
    async def weave_node(db: Session, node_id: str, user_id: str) -> KalpaNode:
        ...
        寫作規範要求 (GEO/AIO 友善架構)：
        1. **核心方案總結**：在文章最開頭，以 100 字內精煉地總結解決方案，不要包含任何「摘要」或「TL;DR」標題。
        2. **嚴禁重複標題**：禁止在輸出內容的最開頭出現與文章標題相同的文字。
        3. **🎯 直接答案片段**：在每個 H2/H3 標題下方，緊接一段 50 字內的精煉回答，直接切入重點。
        4. **對照表格**：包含一個 HTML 表格對比『核心問題』與『優化方案』。
        5. **💡 專家洞察**：在文中插入一個具有深度見解的段落，開頭標註『💡 專家建議：』。
        6. **常見問答 (FAQ)**：在結尾前增加一個『## ❓ 常見問答 (FAQ)』區塊。
