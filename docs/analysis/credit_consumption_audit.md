...
| **writing_optimize** | 5 | **AI 模型** | *(後端 API 備用)* | **SEO 體檢與建議**：目前前端僅顯示數據，尚未呼叫此 AI 優化路由 |
...
| :--- | :--- | :--- | :--- |
| **關鍵字清單建議** | `POST /research/keyword-ideas` | DataForSEO | 此接口獲取大量關鍵字建議，目前不扣點。 |
| **AI 標題建議生成** | `POST /research/generate-titles` | **AI 模型** | 生成 5-10 組 AI 標題建議，目前不扣點。 |
| **文章戰略藍圖生成** | `POST /writing/blueprint` | **AI 模型** | `generate_full` 會用到，但獨立 API 請求目前免費。 |
| **全篇集成審核** | `POST /writing/review` | **AI 模型** | 潤色與優化全文，目前不扣點。 |
| **圖片 Alt/Caption 建議** | `GET /images/metadata-suggestion`| **AI 模型** | AI 產出圖片中繼資料，目前不扣點。 |
| **CMS 文章正式發布** | `POST /cms/publish` | 伺服器 I/O / 第三方 API | 發布至外部平台，目前不扣點。 |
