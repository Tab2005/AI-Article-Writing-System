...
1.  **環境變數 = 預設值**：您在 Zeabur 設定的 `GEMINI_API_KEY` 等變數，會在系統第一次啟動、且資料庫尚無設定時生效。
...
5. **後端啟動指令**（重要）：後端服務中，`zbpack.json` 已自動設定以下指令，Zeabur 會自動套用：
   ```
   uvicorn app.main:app --host 0.0.0.0 --port ${PORT:-8080}
   ```
   > ⚠️ **注意**：Zeabur 使用的預設 port 是 `8080`，而非 `8000`。請勿在 Zeabur 控制台手動把 Start Command 設成 `--port 8000`，否則容器健康檢查失敗會導致部署失敗。
